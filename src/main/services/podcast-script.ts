...
- Meet the target length. About 150 spoken words equal one minute of audio, so 5 minutes needs about 750 words. If no length was requested, aim for 5 minutes.`
...
export function podcastLengthLine(question: string): string {
  ...
  return `Target length: about ${minutes} minute${minutes === 1 ? '' : 's'}, which is about ${minutes * 150} words of spoken script. Stay close to that word count: do not stop far short of it, and do not run far past it.`
}

...
// 'phase' is pure narration between steps ("Understanding the question",
// "Writing the script") — composed by the harness from what it's doing, not
// generated by the model, so it costs zero tokens and is always coherent.
...
export type AgentRunResult = {
  ...
  // Set when the run produced a deliverable instead of a chat answer. The loop
  // itself researched the material and rendered the script/overview (it's in
  // `answer`); the caller (chat:ask) only handles what must outlive the LLM
  // session — the podcast message payload and TTS.
